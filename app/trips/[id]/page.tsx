...
export default async function TripPage({ params }: { params: { id: string } }) {
  ...
  routePoints.push(...trip.pointsOfInterest.map(point => ({ id: point.id, tripId: trip.id, lat: point.latitude, lng: point.longitude, kind: "poi" as const, poiType: point.type, label: point.name || point.type.split("-").map(word => word[0].toUpperCase() + word.slice(1)).join(" "), detail: point.description || `${trip.riverName} point of interest` })));
  return <><div className="detail-top"><Link href="/trips"><ArrowLeft /> Back to trips</Link><div className="detail-actions"><Link className="button secondary" href={`/trips/${trip.id}/edit`}><Pencil /> Edit trip</Link></div></div>
    ...
    <section className="panel trip-map-panel"><div className="panel-head"><div><span className="eyebrow">Route and highlights</span><h2>Trip map</h2></div><span className="muted">{trip.pointsOfInterest.length} {trip.pointsOfInterest.length === 1 ? "point" : "points"} of interest</span></div><GoogleMap points={routePoints} connectRoute /></section>
