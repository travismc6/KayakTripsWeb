...
export function TripPhotos({ tripId, riverName, initialPhotos }: { tripId: string; riverName: string; initialPhotos: Photo[] }) {
  ...
  return <aside className="panel photo-panel">
    ...
    {photos.length ? <div className="photo-grid">{photos.map(photo => <figure key={photo.id}><button type="button" className="photo-open" onClick={() => setSelectedPhoto(photo)} aria-label={`Enlarge ${photo.caption || `${riverName} trip photo`}`}><Image src={photo.url} alt={photo.caption || `${riverName} trip`} fill sizes="(max-width: 800px) 50vw, 240px" /></button><figcaption>{photo.caption && <span>{photo.caption}</span>}<small>Uploaded by {photo.uploadedByName || "Unknown"}</small></figcaption></figure>)}</div> : <div className="photo-empty"><CameraIcon /><h3>No photos yet</h3><p>Add the first memory from this trip.</p></div>}
    ...
    {selectedPhoto && <div className="photo-lightbox" role="dialog" aria-modal="true" aria-label="Enlarged trip photo" onMouseDown={event => { if (event.target === event.currentTarget) setSelectedPhoto(null); }}>
      ...
      <figure><div className="photo-lightbox-image"><Image src={selectedPhoto.url} alt={selectedPhoto.caption || `${riverName} trip`} fill sizes="95vw" priority /></div><figcaption>{selectedPhoto.caption && <span>{selectedPhoto.caption}</span>}<small>Uploaded by {selectedPhoto.uploadedByName || "Unknown"}</small></figcaption></figure>
    </div>}
  </aside>;
}
